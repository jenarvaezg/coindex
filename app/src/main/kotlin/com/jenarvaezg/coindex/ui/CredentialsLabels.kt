...
/**
 * Everything «Credenciales» and the sign-up form say.
 *
 * Two of the screens §5 exempts **by the frequency rule**: they are visited once, a paragraph there
 * costs once, and avoiding a phone call pays for it. The exemption is about how much text is worth,
 * not about where it is written — which is why the paragraphs live here, in a file the tests can
 * read, and not in the screens.
 *
...
/**
 * The screen's own name, printed by the three doors that open it and by its masthead (#521).
 *
 * One string for four places, which is the pairing §14 has held for «Avisos y licencias» since it
 * was written: the row at the foot of «Este teléfono», the row the valuation card grows in the two
 * states that blame the key, the masthead of what they open, and the sync refusals that send the
 * collector here. It stopped being a heading **inside** the screen when it became the screen's
 * name — the masthead says it one line above, and printing it twice is the furniture §5 prices.
 */
 ...
 * it reads as what to expect, and here as the first thing to check — which is why this screen
 ...
 * differently here than on the form that first asked for them would make the collector wonder
...
/**
 * The one word the button needs, now that the screen is named «Credenciales» (#521).
 *
 * It was «Guardar ajustes», which named a screen that no longer exists and, on a screen whose
 * masthead already says its subject, said that subject twice.
 */
 ...
 *
 * It names **what** was saved rather than where, because it is read after the screen has already
 * been left: «Ajustes guardados» was a snackbar naming a place the collector is no longer in.
 */
 ...
 *
 * It lives on this screen and not on «Este teléfono» (#521), because it is what it says it is: it
 * deletes these two fields. The gesture that empties the phone at the foot of the screen the
 * collector visits to sync was a destructive button on a maintenance page.
 */
